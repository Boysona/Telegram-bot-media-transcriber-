...
def set_bot_info():
    ...
    try:
        commands = [
            ...
            telebot.types.BotCommand(command="summarize", description="Summarize last transcription ✍️"), 
...
def handle_summarize(message):
    user_id = str(message.from_user.id)
    if user_id not in last_transcription:
        return bot.send_message(message.chat.id, "No previous transcription found to summarize.")
    
    msg = bot.send_message(
        message.chat.id,
        """Please enter the language you want the summary in (e.g., English, Somali, Arabic) etc.

⚠️ Note: Summarization of transcriptions stored as .txt files is not supported."""
    )
...
def summarize_text(message, user_id):
    lang = message.text.strip()
    original = last_transcription.get(user_id, "")
    prompt = f"Please summarize the following transcription in {lang}:\n\n{original}"
    bot.send_chat_action(message.chat.id, 'typing')
