...
LINK_REGEX = re.compile(r'https?://(?:www\.)?(?:tiktok\.com|vm\.tiktok\.com|youtube\.com|youtu\.be|facebook\.com|instagram\.com|x\.com|twitter\.com|pin\.it)/[^\s]+')

@bot.message_handler(func=lambda m: m.text and LINK_REGEX.search(m.text))
def video_link_handler(message):
    url = LINK_REGEX.search(message.text).group(0)
    markup = telebot.types.InlineKeyboardMarkup()
    if "tiktok.com" in url:
        markup.add(
            telebot.types.InlineKeyboardButton("Download video 📥", callback_data=f"download|tiktok|{url}"),
            telebot.types.InlineKeyboardButton("Transcribe 📝", callback_data=f"transcribe|tiktok|{url}")
        )
        bot.send_message(message.chat.id, "TikTok link detected — choose an action:", reply_markup=markup)
    elif "youtube.com" in url or "youtu.be" in url:
        markup.add(
            telebot.types.InlineKeyboardButton("Download video 📥", callback_data=f"download|youtube|{url}"),
            telebot.types.InlineKeyboardButton("Transcribe audio 🎧", callback_data=f"transcribe|youtube|{url}")
        )
        bot.send_message(message.chat.id, "YouTube link detected — choose an action:", reply_markup=markup)
    elif "facebook.com" in url:
        markup.add(
            telebot.types.InlineKeyboardButton("Download video 📥", callback_data=f"download|facebook|{url}"),
            telebot.types.InlineKeyboardButton("Transcribe audio 🎧", callback_data=f"transcribe|facebook|{url}")
        )
        bot.send_message(message.chat.id, "Facebook link detected — choose an action:", reply_markup=markup)
    elif "instagram.com" in url:
        markup.add(
            telebot.types.InlineKeyboardButton("Download video 📥", callback_data=f"download|instagram|{url}"),
            telebot.types.InlineKeyboardButton("Transcribe audio 🎧", callback_data=f"transcribe|instagram|{url}")
        )
        bot.send_message(message.chat.id, "Instagram link detected — choose an action:", reply_markup=markup)
    elif "x.com" in url or "twitter.com" in url:
        markup.add(
            telebot.types.InlineKeyboardButton("Download video 📥", callback_data=f"download|twitter|{url}"),
            telebot.types.InlineKeyboardButton("Transcribe audio 🎧", callback_data=f"transcribe|twitter|{url}")
        )
        bot.send_message(message.chat.id, "X (Twitter) link detected — choose an action:", reply_markup=markup)
    elif "pin.it" in url:
        bot.send_message(message.chat.id, "Pin.it links are not supported for video download or transcription.")
    else:
        bot.send_message(message.chat.id, "Unsupported video link.")

@bot.callback_query_handler(func=lambda c: c.data.startswith("download|"))
def callback_download(call):
    global total_tiktok_downloads, total_videos
    action, platform, url = call.data.split("|", 2)
    ...
    try:
        ...
        logging.error(f"{platform.capitalize()} download error: {e}")
        bot.send_message(call.message.chat.id, f"⚠️ Failed to download {platform.capitalize()}.")
    finally:
        if 'path' in locals() and os.path.exists(path):
            os.remove(path)

@bot.callback_query_handler(func=lambda c: c.data.startswith("transcribe|"))
def callback_transcribe(call):
    action, platform, url = call.data.split("|", 2)
    bot.send_chat_action(call.message.chat.id, 'typing')
    try:
        ydl_opts = {
        ...
        if len(transcription) > 4000:
            fn = f'{platform}_transcription.txt'
        ...
        logging.error(f"{platform.capitalize()} transcribe error: {e}")
        bot.send_message(call.message.chat.id, f"⚠️ Failed to transcribe {platform.capitalize()}.")
    finally:
