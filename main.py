...
1. **Join the Channel:** @mediatranscriber  
2. **Send a File:** voice, audio, video, or TikTok URL  
3. **Transcription:** automatic & fast  
4. **Commands:**  
   • /start /status /help /translate /summarize"""
    )
...
def status_handler(message):
    ...
    total_time_str = format_timedelta(total_processing_time)

    text = (
        "📊 Overall Statistics\n\n"
        "👥 User Statistics\n"
        f"▫️ Total Users Today: {users_today}\n\n"
        ...
        f"⏱️ Total Processing Time: {total_time_str}\n\n"
        "⸻\n\n"
...
def callback_download_tiktok(call):
    ...
    try:
        ...
        logging.exception("TikTok download error")
...
def callback_transcribe_tiktok(call):
    ...
    try:
        ...
        global total_videos, total_files_processed, total_processing_time
        total_videos += 1
        ...
        logging.exception("TikTok transcribe error")
...
def handle_media(message):
    ...
    if message.voice:
        ...
        mime, _ = guess_type(message.document.file_name or "")
    ...
    if not file_obj:
        return bot.send_message(
            message.chat.id,
            "⚠️ I couldn't find an audio or video file to transcribe. "
            "Please send a voice note, audio/video file, or a direct TikTok link."
        )

    if file_obj.file_size > FILE_SIZE_LIMIT:
        return bot.send_message(message.chat.id, "⚠️ File too large (max 20 MB).")
    ...
    try:
        ...
        global processing_start_time, total_files_processed, total_audio_files, total_voice_clips, total_videos, total_processing_time
        processing_start_time = datetime.now()
        transcription = transcribe(local_path)
        if transcription is None:
            raise RuntimeError("Whisper returned None")
        ...
        if message.voice:
            total_voice_clips += 1
        elif message.audio or (message.document and mime.startswith('audio/')):
            total_audio_files += 1
        else:
            total_videos += 1
        ...
        bot.send_message(message.chat.id, "⚠️ An error occurred during transcription. Please try again.")
    finally:
...
def do_translate(message, uid):
    ...
    if len(translated) > 2000:
        ...
        bot.send_chat_action(message.chat.id, 'upload_document')
        with open(fn, 'rb') as doc:
            bot.send_document(message.chat.id, doc)        bot.send_message(message.chat.id, translated)
...
def fallback(message):
    ...
    if not check_subscription(message.from_user.id):
        return send_subscription_message(message.chat.id)
    bot.send_message(message.chat.id, "⚠️ Please send only voice, audio, video, or a TikTok link.")
