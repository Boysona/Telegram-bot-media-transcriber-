...
def set_bot_info():
    ...
    bot.set_my_short_description("Upload media or links and get fast transcriptions!")
    bot.set_my_description(
        "This bot transcribes, translates, and summarizes voice messages, audio files, videos, TikTok links, and YouTube links—for free!"
    )
...
def help_handler(message):
    help_text = (
        ...
        "2. **Send Media or Link:** Send a voice message, audio file, video file, TikTok URL, YouTube URL, or YouTube shorts URL.\n"
        "3. **Get Transcription:** The bot will transcribe your input and send back the text. Long transcriptions will be delivered as a text file.\n"
        "4. **Optional Actions:** After you receive the transcription, you can translate or summarize it using the provided buttons.\n\n"
        "**Available Commands:**\n"
...
def privacy_notice_handler(message):
    privacy_text = (
        "**Privacy Notice**\n\n"
        "**Data We Process:**\n"
        "- **Media Files:** Any media or links you send are processed temporarily for transcription and then deleted immediately.\n"
        "- **Transcriptions:** Text transcriptions are stored in memory only until you choose to translate or summarize. We do not store them permanently.\n"
        ...
        "- **Language Preferences:** We save your chosen language for translations and summaries.\n\n"
        ...
        "- Transcription and translation are performed using Whisper and Gemini APIs; we do not store your inputs permanently.\n\n"
        "**Data Retention:**\n"
        "- Media files are deleted immediately after transcription.\n"
        "- Transcriptions are held in memory temporarily.\n"
        "- User IDs and language preferences are kept until you stop using the bot or request deletion.\n\n"
...
def status_handler(message):
    ...
    text = (
        "📊 **Bot Statistics**\n\n"
        f"• Total Users Active Today: {active_today}\n\n"
        f"• Total Files Processed: {total_files_processed}\n"
        f"• Audio Files: {total_audio_files}\n"
        f"• Voice Clips: {total_voice_clips}\n"
        f"• Videos: {total_videos}\n"
        f"• TikTok Downloads: {total_tiktok_downloads}\n\n"
        f"⏱ Total Processing Time: {hours}h {minutes}m {seconds}s"
    )
...
def callback_transcribe_tiktok(call):
    ...
    try:
        ...
        if len(transcription) > 4000:
            ...
            bot.send_chat_action(call.message.chat.id, "typing")
            bot.send_message(call.message.chat.id, transcription, reply_markup=buttons)
    except Exception as e:
...
def callback_set_language(call):
    ...
    bot.edit_message_text(
        ...
        text=f"✅ Your preferred language is set to **{lang}**",
        parse_mode="Markdown"
    )
...
def fallback(message):
    ...
    if not check_subscription(message.from_user.id):
        return send_subscription_message(message.chat.id)
    bot.send_message(message.chat.id, "Please send only voice, audio, video, TikTok link, or YouTube link.")
