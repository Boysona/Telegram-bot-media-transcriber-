...
def set_bot_info():
    ...
    bot.set_my_description(
        description="Download & transcribe media & TikTok links; translate/summarize long transcripts into TXT files—fast, easy, free!"
    )
    bot.set_my_short_description(
        short_description="Media & TikTok downloader + transcription + TXT translate/summarize"
    )
...
def start_handler(message):
    ...
    if message.from_user.id == ADMIN_ID:
        ...
        bot.send_message(
            message.chat.id,
            f"👋 Hello {name}!\n\n"
            "• Send a voice, audio, video file, or a TikTok link.\n"
            "• Tap 📥 to download TikTok, 📝 to transcribe it."
        )
...
def help_handler(message):
    help_text = (
        ...
        "3. *Download TikTok:* tap 📥 (video + caption)\n"
        "4. *Transcribe TikTok:* tap 📝 (saved for translate/summarize)\n"
        "5. *Translate:* /translate (TXT file if >2000 chars)\n"
        "6. *Summarize:* /summarize (TXT file if >2000 chars)\n"
        "7. *Status:* /status\n\n"
...
def status_handler(message):
    ...
    text = (
        ...
        f"🎵 *Audio:* {total_audio_files}\n"
        f"🎙️ *Voice:* {total_voice_clips}\n"
        f"🎬 *Video:* {total_videos}\n"
        f"⏱️ *Total time:* {format_timedelta(total_processing_time)}"
    )
...
def handle_file(message):
    ...
    try:
        ...
        last_transcription[uid] = transcription
        # update statistics omitted for brevity...
        if len(transcription) > 2000:
