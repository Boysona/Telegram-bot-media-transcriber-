...
‼️ 𝗔𝗳𝘁𝗲𝗿 𝗷𝗼𝗶𝗻𝗶𝗻𝗴, 𝘀𝗲𝗻𝗱 /start 𝘁𝗼 𝗰𝗼𝗻𝘁𝗶𝗻𝘂𝗲""",
        reply_markup=markup
    )
...
def callback_transcribe_tiktok(call):
    global total_files_processed, total_videos, processing_start_time, total_processing_time
    _, url = call.data.split("|", 1)
    bot.send_chat_action(call.message.chat.id, 'typing')
    try:
        ydl_opts = {
...
def handle_file(message):
    global total_files_processed, total_audio_files, total_voice_clips, total_videos, processing_start_time, total_processing_time
    update_user_activity(message.from_user.id)
    if not check_subscription(message.from_user.id):
        return send_subscription_message(message.chat.id)
