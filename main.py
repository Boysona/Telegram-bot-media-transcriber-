...
def set_bot_info():
    commands = [
        ...
        telebot.types.BotCommand("status", "Show bot statistics (admin only)")
    ]
    bot.set_my_commands(commands)
    bot.set_my_description("A Telegram bot that transcribes voice, video, and audio messages.")
    bot.set_my_short_description("Transcribes media files.")
    logging.info("Bot commands, description, and about info set successfully.")
...
def send_subscription_message(chat_id):
    ...
    bot.send_message(chat_id, "⚠️ first Please join the channel to continue using this bot!", reply_markup=markup)

def get_user_counts():
    total_users = len(user_data)
    now = datetime.now()
    monthly_active_users = sum(1 for user_id, last_active in user_data.items() if is_active_within(last_active, 30))
    weekly_active_users = sum(1 for user_id, last_active in user_data.items() if is_active_within(last_active, 7))
    return total_users, monthly_active_users, weekly_active_users

def is_active_within(last_active_str, days):
    if last_active_str:
        last_active = datetime.fromisoformat(last_active_str)
        time_difference = datetime.now() - last_active
...
def help_handler(message):
    help_text = """Here's how to use the Media Transcriber Bot:

1. **Join the Channel:** Make sure you've joined our channel @mediatranscriber to use the bot.

2. **Send a Media File:** You can send voice messages, audio files, video files, or video notes directly to the bot.

3. **Transcription:** The bot will automatically process your file and transcribe the audio into text. This might take a few moments depending on the file size.

4. **Receive Transcription:** Once the transcription is complete, the bot will send you the text. If the transcription is too long, it will be sent as a separate text file.
...
- The accuracy of the transcription depends on the clarity of the audio.
...
"""
        ...
        bot.send_message(message.chat.id, "⚠️ This command is only for administrators.")
...
def handle_file(message):
    ...
    try:
        ...
        if transcription:
            ...
            bot.send_message(message.chat.id, "⚠️ I could not transcribe the audio please clear your audio.")
