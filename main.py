...
import asyncio # Keep asyncio for MSSpeech
import threading
import time
...
def start_handler(message):
    ...
    # Ensure TTS mode is OFF on /start
    ...
    if message.from_user.id == ADMIN_ID:
        ...
        # Prompt user to select media-language directly on /start
        markup = generate_language_keyboard("set_media_lang")
        bot.send_message(
            message.chat.id,
            "Please choose the language of the your audio files for transcription using the buttons below.",
            reply_markup=markup
        )
...
def help_handler(message):
    ...
    # Ensure TTS mode is OFF on /help
...
def privacy_notice_handler(message):
    ...
    # Ensure TTS mode is OFF on /privacy
    user_tts_mode[user_id] = None
...
def status_handler(message):
    ...
    # Ensure TTS mode is OFF on /status
    user_tts_mode[user_id] = None
...
def handle_file(message):
    ...
    # Ensure user has set media_language
...
def process_media_file(message, stop_typing, file_obj, type_str):
    ...
    try:
        ...
        media_lang_code = get_lang_code(media_lang_name) if media_lang_name else None
...
# --- REMOVED: transcribe_audio_with_chunks function (not needed with AssemblyAI) ---
...
LANGUAGES = {
    ...
    "Português 🇵🇹": "pt", "हिन्दी 🇮🇳": "hi", "Somali 🇸🇴": "so" # Added Somali
}
...
# --- NEW: TTS VOICES BY LANGUAGE (unchanged list, as this is for MSSpeech) ---
...
def on_tts_back_to_languages(call):
    ...
    # Going back: reset user_tts_mode (voice no longer selected)
...
def select_language_command(message):
    ...
    # Ensure TTS mode is OFF on /language
    user_tts_mode[uid] = None
...
def callback_set_language(call):
    ...
    # Ensure TTS mode is OFF when setting language
    user_tts_mode[uid] = None
...
def select_media_language_command(message):
    ...
    # Ensure TTS mode is OFF on /media_language
    user_tts_mode[uid] = None
...
def callback_set_media_language(call):
    ...
    # Ensure TTS mode is OFF when setting media language
    user_tts_mode[uid] = None
...
def button_translate_handler(call):
    ...
    # Ensure TTS mode is OFF when using translate button
    user_tts_mode[uid] = None
...
def button_summarize_handler(call):
    ...
    # Ensure TTS mode is OFF when using summarize button
    user_tts_mode[uid] = None
...
def callback_translate_to(call):
    ...
    # Ensure TTS mode is OFF when using translate_to callback
    user_tts_mode[uid] = None
...
def callback_summarize_in(call):
    ...
    # Ensure TTS mode is OFF when using summarize_in callback
    user_tts_mode[uid] = None
...
def handle_translate(message):
    ...
    # Ensure TTS mode is OFF on /translate
    user_tts_mode[uid] = None
...
def handle_summarize(message):
    ...
    # Ensure TTS mode is OFF on /summarize
    user_tts_mode[uid] = None
...
def handle_text_for_tts_or_fallback(message):
    ...
    # If user_tts_mode[uid] is set (voice chosen), or user has a saved voice, do TTS
...
def fallback_non_text_or_media(message):
    ...
    # Ensure TTS mode is OFF when a non-text/non-media message is sent
    user_tts_mode[uid] = None
