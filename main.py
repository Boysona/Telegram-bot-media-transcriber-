...
def set_bot_info():
    ...
    bot.set_my_short_description(
        "Instantly transcribe, summarize, and translate your audio and video files with this free bot!"
    )

    # Full description (What can this bot do?)
    bot.set_my_description(
        """This bot helps you quickly transcribe voice messages, audio files, videos, and even TikTok links.
    
    ✨ It's completely free and supports multiple languages for transcription, summarization, and translation.
    
    Please note: The bot **does not support files larger than 20MB**.

    Start transcribing, summarizing, and translating your media today! 🚀"""
    )
...
def start_handler(message):
    ...
    if message.from_user.id == ADMIN_ID:
        ...
        bot.send_message(
            message.chat.id,
            f"""👋 Welcome, {name}!
I can help you transcribe, summarize, and translate your audio and video content.
...
**Important:** Please note that I do not support files larger than **20MB**.
...
5.  **File Size Limit:** Please note that the bot **does not support files larger than 20MB**.
6.  **Commands:**
...
Enjoy transcribing and managing your media quickly and easily!"""
    )
...
def fallback(message):
    update_user_activity(message.from_user.id)
    bot.send_message(message.chat.id, "Please send only voice, audio, video, or a TikTok video link. Remember, files larger than **20MB** are not supported.")
