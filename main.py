...
# In-memory chat history and last 10 transcriptions store
user_memory = {}
# Now last_transcription maps uid -> list of up to 10 transcription strings
last_transcription = {}
...
6. **Commands:**
   ...
   - `/history`: See your last 10 transcriptions (optional).
...
def callback_transcribe_tiktok(call):
    ...
    try:
        ...
        # ——————————————
        # Ku dar transcription liiska 10-kii ugu dambeeyey
        last_transcription.setdefault(uid, [])
        last_transcription[uid].append(transcription)
        if len(last_transcription[uid]) > 10:
            last_transcription[uid].pop(0)
        # ——————————————
...
def button_translate_handler(call):
    uid = str(call.from_user.id)
    # Hubi in transcription’s list uusan madhnayn
    if uid not in last_transcription or len(last_transcription[uid]) == 0:
...
def handle_translate(message):
    ...
    prompt_msg = bot.send_message(
        message.chat.id,
        "Send me the name of the language you want the text to be translated into (e.g. Spanish, Arabic):"
    )
    bot.register_next_step_handler(prompt_msg, lambda resp: do_translate(resp, uid))

def do_translate(message, uid):
    lang = message.text.strip()
    # Ka qaado transcription kii ugu dambeeyey
    original = last_transcription[uid][-1]
...
def show_history(message):
    uid = str(message.from_user.id)
    if uid not in last_transcription or len(last_transcription[uid]) == 0:
        return bot.send_message(message.chat.id, "❌ No transcription history found.")
    teks_list = last_transcription[uid]
    text = "🕘 Waa taariikhda 10-kii transcripts ee ugu dambeeyey:\n\n"
    # Muujinta liiska oo kooban (50 xaraf kaliya per transcription)
    for i, teks in enumerate(teks_list, start=1):
        snippet = teks[:50].replace("\n", " ") + ("..." if len(teks) > 50 else "")
        text += f"{i}. {snippet}\n"
    bot.send_message(message.chat.id, text)
