...
def set_bot_info():
    ...
    bot.set_my_short_description(
        "Kani waa bot kuu fududeynaya inaad falanqeyso, soo koobto, iyo tarjunto faylashaada maqalka iyo muuqaalka si degdeg ah oo bilaash ah!"
    )

    # Full description (What can this bot do?)
    bot.set_my_description(
        """**Sii Daaha Ka Qaad Awooda Warbaahintaada!** 🚀
Botkan wuxuu si degdeg ah kuu falanqeynayaa (transcribes), kuu soo koobayaa (summarizes), kuna tarjumayaa (translates) farriimaha codka ah, fiidyowyada TikTok, faylasha maqalka ah, iyo fiidyowyada caadiga ah. Dhammaan bilaash, luuqado kala duwanna waa la heli karaa!

Ma rabtaa in aad fududayso dhagaysiga ama daawashada, ama in aad fahamto waxa ku jira luqad kale? Botkan ayaa kuu ah xalka! Ku raaxayso adeeg bilaash ah oo degdeg ah.

**Qaybaha Muhiimka Ah:**
•  **Falanqayn Degdeg Ah:** U beddel codka qoraal daqiiqado gudahood.
•  **Soo Koobid:** Soo koob qoraalada dhaadheer si aad u hesho qodobada muhiimka ah.
•  **Tarjumaad Luuqado Badan:** Ku tarjun qoraalka luuqad kasta oo aad doorato.
•  **TikTok Download & Falanqayn:** Soo deji ama falanqee fiidyowyada TikTok si fudud.
•  **Bilaash & Fudud In La Isticmaalo:** Ma jiro wax lacag ah ama qalab adag.

**Bilow Hadda oo Arko Farqiga!** 👇
Haddii aad leedahay faylal warbaahineed oo u baahan falanqayn, soo koobid, ama tarjumaad, botkan ayaa kuu fududaynaya. Nala soo xiriir haddii aad hayso wax su'aalo ah. Enjoy! ✨"""
    )
...
def start_handler(message):
    ...
    if message.from_user.id == ADMIN_ID:
        ...
        if user_id not in user_language_settings:
            ...
            # Add Russian, Uzbek, Arabic, English as top 3+1 options
            markup.add(
                InlineKeyboardButton("Русский 🇷🇺", callback_data="select_initial_lang|Russian"),
                InlineKeyboardButton("O'zbek 🇺🇿", callback_data="select_initial_lang|Uzbek"),
                InlineKeyboardButton("العربية 🇸🇦", callback_data="select_initial_lang|Arabic"),
                InlineKeyboardButton("English 🇬🇧", callback_data="select_initial_lang|English")
            )
            # You might want to add a few more common ones or paginate if too many
            bot.send_message(
                message.chat.id,
                "👋🏻 Welcome! Please select your preferred language:\n\n"
                "👋🏻 Ассалому алейкум! Илтимос, афзал кўрган тилингизни танланг:\n\n"
                "👋🏻 مرحباً! الرجاء اختيار لغتك المفضلة:\n\n"
                "👋🏻 Kusoo Dhawoow! Fadlan dooro luqadaada aad jeceshahay:",
                reply_markup=markup
            )
        else:
            bot.send_message(
...
@bot.callback_query_handler(func=lambda c: c.data.startswith("select_initial_lang|"))
def callback_select_initial_language(call):
...
1.  **Send a File/Link:** Send a voice message, audio, video, or a TikTok URL.
2.  **Receive Transcription:** The bot will process your input and send back the transcribed text. Long transcriptions will be sent as a text file.
    * **Note:** Files larger than 20MB cannot be processed.
3.  **TikTok Options:** For TikTok links, you can choose to **Download** the video or **Transcribe** it.
