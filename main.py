...
import imageio_ffmpeg as ffmpeg_static
...
# AssemblyAI API key (set this as an environment variable in your hosting environment)
...
# Google Gemini API key (for translation/summarization)
...
3.  **Data Sharing:**,
    ...
    * Transcription, translation, and summarization are performed using integrated AI models (AssemblyAI for transcription and Gemini API). Your text input to these models is handled according to their respective privacy policies, but we do not store this data after processing.,
...
def handle_file(message):
    ...
    local_input = os.path.join(DOWNLOAD_DIR, f"{uuid.uuid4()}{ext}")
    ...
    try:
        ...
        transcription = transcribe(local_input) or ""
...
# List of common languages with emojis (Somali moved higher)
LANGUAGES = [
    {"name": "English", "flag": "🇬🇧"},
    {"name": "Chinese", "flag": "🇨🇳"},
    {"name": "Spanish", "flag": "🇪🇸"},
...
def convert_to_wav(input_path: str) -> str:
    """
    Converts any supported audio/video file to a 16kHz, mono WAV using ffmpeg-static.
    Returns the path to the converted WAV file.
    """
    base, _ = os.path.splitext(input_path)
    wav_path = f"{base}.wav"
    ffmpeg_exe = ffmpeg_static.get_ffmpeg_exe()
...
def upload_to_assemblyai(filepath: str) -> str:
    """
    Uploads the file to AssemblyAI and returns the upload_url.
    """
...
def request_transcript(upload_url: str) -> str:
    """
    Requests a transcript from AssemblyAI for the given upload_url.
...
def transcribe(path: str) -> str | None:
    """
    Transcribes the given audio/video file using AssemblyAI.
    Converts the file to WAV (16kHz mono) via ffmpeg-static, uploads to AssemblyAI,
    requests a transcript, and polls for completion.
    """
    if not ASSEMBLYAI_API_KEY:
        logging.error("AssemblyAI API key is not set.")
        return None
    ...
    try:
        ...
        upload_url = upload_to_assemblyai(wav_path)
        ...
        logging.error(f"Transcription error (AssemblyAI): {e}")
        return None
