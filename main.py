...
# === ADDED FROM CODE 1: AI MEMORY & TRANSLATION ===

# In-memory per-user chat history for Gemini
user_memory = {}
# Store last transcription per user for translation
last_transcription = {}
...
def check_subscription(user_id):
    try:
        ...
        # Return True if we can't check - prevents blocking users incorrectly
        return True
...
def status_handler(message):
    total_users, monthly_active, weekly_active = get_user_counts()
    status_text = f"""Today's Activity -- 🗓️

📊 Users Today: {total_users}
📊 Monthly Active Users: {monthly_active}
...
def handle_file(message):
    ...
    try:
        ...
        # Store last transcription for /translate
        if transcription:
...
def translate_text(message, user_id):
    ...
    # Check if original text is too long for translation
    if len(original) > 4000:
        return bot.send_message(
            message.chat.id,
            "⚠️ The text is too long for translation. Please try with a shorter audio clip."
        )
    ...
    if len(translation) > 2000:
        filename = f"translation_{lang}.txt"
        with open(filename, 'w', encoding='utf-8') as f:
            f.write(translation)
        with open(filename, 'rb') as f:
            bot.send_document(message.chat.id, f, caption=f"Translation to {lang}")
        os.remove(filename)
    else:
        bot.send_message(
            message.chat.id,
...
def set_webhook():
    ...
    if webhook_url:
        ...
        return 'Webhook URL not provided. Use ?url=your-webhook-url parameter', 400
...
if __name__ == "__main__":
    ...
    webhook_url = os.environ.get('WEBHOOK_URL', "https://telegram-bot-media-transcriber.onrender.com")
