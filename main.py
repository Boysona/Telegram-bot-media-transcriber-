...
def set_bot_info():
    commands = [
        ...
        telebot.types.BotCommand("translate", "Translate last transcription")  # NEW
    ]
...
def handle_start(message):
    ...
    if message.from_user.id == ADMIN_ID:
        ...
        bot.send_message(message.chat.id, "Waad ku soo dhowaaday! Soo dir warbaahin (cod/video) ama wax su'aal ah (qoraal) si aad u bilowdo.")

@bot.message_handler(commands=['help'])
def handle_help(message):
    bot.send_message(message.chat.id, """**Sida Loo Isticmaalo Bot-ka:**

• Soo dir cod, maqal, ama muuqaal si laguu qoro.
• Ama soo dir qoraal si aad ula sheekeysato Gemini AI.
• /reset - Tirtir xusuusta AI
• /translate - Turjun qoraalkii ugu dambeeyay
• /status - Tirakoob (maamulayaasha kaliya)

Qiyaasta faylka ugu badan: 20MB
Ku biir channel-ka waa lagama maarmaan: @mediatranscriber
""", parse_mode="Markdown")
...
def handle_status(message):
    ...
    bot.send_message(message.chat.id, f"""📊 **Tirakoob:**
• Isticmaalayaasha Guud: {total}
• Kuwa Fircoon Bishii: {monthly}
• Kuwa Fircoon Toddobaadkii: {weekly}
• Faylal la Habeeyay: {total_files_processed}
• Waqtiga la Qaatay: {format_timedelta(total_processing_time)}""", parse_mode="Markdown")

@bot.message_handler(commands=['reset'])
def reset_memory(message):
    user_memory.pop(message.from_user.id, None)
    bot.send_message(message.chat.id, "Xusuusta AI waa la tirtiray.")

@bot.message_handler(commands=['translate'])  # NEW
def handle_translate(message):
    user_id = message.from_user.id
    if user_id not in last_transcription:
        return bot.send_message(message.chat.id, "Ma jiro qoraal transcription ah oo hore laguugu sameeyay.")

    msg = bot.send_message(message.chat.id, "Luqaddee ayaad rabtaa in laguugu turjumo? (Tusaale: Somali, Arabic, English...)")
    bot.register_next_step_handler(msg, lambda m: translate_text(m, user_id))

def translate_text(message, user_id):  # NEW
    lang = message.text.strip()
    original_text = last_transcription.get(user_id, "")
...
def send_broadcast(message):
    ...
    bot.send_message(message.chat.id, "Soo dir fariinta baahinta hadda:")
...
def handle_broadcast(message):
    ...
    for user_id in user_data:
        try:
            ...
            fail += 1
    bot.send_message(message.chat.id, f"Waa la dhammeeyay!\nGuuleystay: {success}\Fashilmay: {fail}")
...
def handle_media(message):
    ...
    if file_size > FILE_SIZE_LIMIT:
        return bot.send_message(message.chat.id, "⚠️ Faylka waa mid aad u weyn! Ugu badnaan 20MB.")
    ...
    if transcription:
        last_transcription[message.from_user.id] = transcription  # NEW
        ...
        bot.send_message(message.chat.id, "❗ Ma suurtoobin in la qoro. Fadlan isku day mar kale.")
