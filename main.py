...
# Regex for TikTok and YouTube links
...
This bot transcribes voice messages, audio files, videos, TikTok video links, and YouTube links.

one (1).  **Join the Channel:** Make sure you've joined our channel: https://t.me/transcriberbo,
two (2).  **Send a File/Link:** Send a voice message, audio, video, TikTok URL, or YouTube URL,
three (3).  **Receive Transcription:** The bot will process your input and send back the transcribed text. Long transcriptions will be sent as a text file,
four (4).  **Post-Transcription Actions:** After transcription, you'll see options to **Translate** or **Summarize** the text,
five (5).  **Commands:**
...
one (1).  **Data We Process:**
    * **Media Files:** Voice messages, audio files, video files, TikTok links, and YouTube links you send are temporarily processed for transcription. These files are **deleted immediately** after processing. We do not store your media content,
...
def youtube_link_handler(message):
    ...
    markup = telebot.types.InlineKeyboardMarkup()
    markup.add(
        telebot.types.InlineKeyboardButton("Transcribe YouTube", callback_data=f"transcribe_youtube|{url}|{message.message_id}")
    )
    bot.send_message(message.chat.id, "YouTube link detected—xulo inaad transcribe garayso:", reply_markup=markup)
...
def fallback(message):
    ...
    if not check_subscription(message.from_user.id):
        return send_subscription_message(message.chat.id)
    bot.send_message(message.chat.id, "Please send only voice, audio, video, or a TikTok/YouTube video link.")
