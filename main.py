...
# In-memory chat history and ALL transcriptions store (changed from last_transcription)
user_memory = {}

# Store all transcriptions per user
# Each user_id will map to a list of dictionaries, where each dict contains 'timestamp' and 'text'
all_transcriptions = {}

def add_transcription_to_history(user_id, transcription_text):
    if user_id not in all_transcriptions:
        all_transcriptions[user_id] = []
    
    # Store transcription with a timestamp for potential future features (e.g., retrieving by date)
    all_transcriptions[user_id].append({
        'timestamp': datetime.now().isoformat(),
        'text': transcription_text
    })

def get_last_transcription(user_id):
    if user_id in all_transcriptions and all_transcriptions[user_id]:
        return all_transcriptions[user_id][-1]['text'] # Get the text of the most recent transcription
    return None
# --- END OF MODIFICATION ---
...
def set_bot_info():
    commands = [
        ...
        telebot.types.BotCommand("history", "Show your transcription history") # New command
    ]
...
6.  **Commands:**
    ...
    -   `/history`: View your past transcriptions. (NEW!)
...
def callback_transcribe_tiktok(call):
    ...
    try:
        ...
        # --- MODIFIED: Store transcription in all_transcriptions ---
        add_transcription_to_history(uid, transcription)
        # --- END OF MODIFICATION ---
...
def button_translate_handler(call):
    uid = str(call.from_user.id)
    # --- MODIFIED: Use get_last_transcription ---
    if not get_last_transcription(uid):
...
def do_translate_with_saved_lang(message, uid, lang):
    # --- MODIFIED: Use get_last_transcription ---
    original = get_last_transcription(uid)
    if not original:
...
def handle_history(message):
    ...
    user_transcriptions = all_transcriptions.get(uid, [])
    if not user_transcriptions:
        bot.send_message(message.chat.id, "You don't have any past transcriptions yet.")
        return

    history_text = "📚 Your Transcription History:\n\n"
    for i, entry in enumerate(user_transcriptions):
        timestamp_str = datetime.fromisoformat(entry['timestamp']).strftime("%Y-%m-%d %H:%M:%S")
        snippet = entry['text'][:100] + ("..." if len(entry['text']) > 100 else "")
        history_text += f"{i+1}. **{timestamp_str}**: {snippet}\n"

    # Offer to send a file if history is too long
    if len(history_text) > 4000:
        fn = 'transcription_history.txt'
        with open(fn, 'w', encoding='utf-8') as f:
            full_history = ""
            for i, entry in enumerate(user_transcriptions):
                timestamp_str = datetime.fromisoformat(entry['timestamp']).strftime("%Y-%m-%d %H:%M:%S")
                full_history += f"--- Transcription {i+1} ({timestamp_str}) ---\n"
                full_history += entry['text'] + "\n\n"
            f.write(full_history)
        bot.send_chat_action(message.chat.id, 'upload_document')
        with open(fn, 'rb') as doc:
            bot.send_document(message.chat.id, doc, caption="Here's your complete transcription history.")
        os.remove(fn)
    else:
