...
# Seed langdetect to make results reproducible
...
user_transcriptions_lang = {}       # Format: {user_id: {message_id: "detected_lang"}}
...
def handle_file(message):
    ...
    try:
        ...
        # Save transcription and detected language
...
def transcribe(path: str) -> str | None:
    """
    Transcribes the given audio/video file using Google Speech Recognition.
    Converts the file to WAV (16kHz mono) via ffmpeg-static, then uses speech_recognition.
    Detects language of the resulting text via langdetect.
    """
    try:
        # Convert to WAV
        ...
        # Google Web Speech API (free tier, limited to short files)
        text = recognizer.recognize_google(audio_data)

        # Attempt to detect language of transcribed text
        try:
            lang_code = detect(text)
        except Exception:
            lang_code = "unknown"

        # Store detected language in memory
        # We do not have the message_id or user_id here, so storing happens in caller
        # Return a tuple of (text, lang_code) for caller to handle
        # Clean up WAV file
